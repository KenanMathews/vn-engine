...
const SCRIPT_LIBRARY: Script[] = [
  // Featured Scripts
  {
    ...
    description: 'Complete demonstration of all template helpers and VN Engine features',
    category: 'featured',
    icon: '🎭',
  ...
  },
  {
    ...
    description: 'Full RPG experience with character creation, combat, and quest system',
    category: 'featured',
    icon: '⚔️',
    difficulty: 'Advanced',
  ...
  // Example Scripts
  {
    ...
    description: 'The simplest possible VN script - perfect for getting started',
    category: 'example',
    icon: '👋',
    difficulty: 'Beginner',
  ...
  // DLC Scripts (can be added as DLC to compatible base scripts)
  {
    ...
    description: 'Experience a magical winter festival with holiday-themed quests',
    category: 'demo',
    icon: '🎄',
    difficulty: 'Intermediate',
...
class VNEngineDemo {
  ...
  private initializeElements(): void {
    ...
    this.sceneSelect = document.getElementById('scene-select') as HTMLSelectElement
    this.yamlEditor = document.getElementById('yaml-editor') as HTMLTextAreaElement
  ...
  private setupEventListeners(): void {
    ...
    this.vnEngine.on('error', (error) => this.showNotification(`Error: ${error}`, 'error'))
  ...
  private renderLibrary(): void {
    const categories = [
      { id: 'featured', name: '🌟 Featured', scripts: SCRIPT_LIBRARY.filter(s => s.category === 'featured') },
      { id: 'demo', name: '📚 Demos', scripts: SCRIPT_LIBRARY.filter(s => s.category === 'demo') },
      { id: 'example', name: '📝 Examples', scripts: SCRIPT_LIBRARY.filter(s => s.category === 'example') },
      { id: 'advanced', name: '🔧 Advanced', scripts: SCRIPT_LIBRARY.filter(s => s.category === 'advanced') }
    ]
    ...
    categories.forEach(category => {
      ...
      html += `<div style="margin-bottom: 24px;">
        <h3 style="margin-bottom: 12px; color: #495057; font-size: 16px;">${category.name}</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px;">
          ${category.scripts.map(script => this.renderScriptCard(script)).join('')}
        </div>
      </div>`
    })
  ...
  private renderScriptCard(script: Script): string {
    ...
    return `
      <div class="script-card ${isCurrent ? 'current' : ''} ${script.category === 'featured' ? 'featured' : ''}">
        <div class="script-meta">
          <div class="script-icon">${script.icon}</div>
          <div class="script-badge ${badgeClass}">${isCurrent ? '✓ Current' : badge}</div>
        </div>
        <div class="script-title">${script.name}</div>
        <div class="script-description">${script.description}</div>
        <div class="script-features">
          <strong style="color: ${difficultyColors[script.difficulty]};">${script.difficulty}</strong> • 
          ${script.features.slice(0, 2).join(' • ')}${script.features.length > 2 ? ' • ...' : ''}
        </div>
  ...
  private filterLibrary(query: string): void {
    ...
    cards.forEach(card => {
      ...
      const features = card.querySelector('.script-features')?.textContent?.toLowerCase() || ''
  ...
  private async validateDLC(scriptId: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Cannot load script: ${response.status}`)
  ...
  private async installDLC(scriptId: string): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showNotification(`❌ Failed to install DLC: ${result.error?.message}`, 'error')
      }
  ...
  public async loadScript(scriptId: string): Promise<void> {
    ...
    try {
      ...
      this.showNotification(`Failed to load ${script.name}: ${error.message}`, 'error')
    }
  }
  ...
  private updateUI(): void {
    ...
    if (this.vnEngine.getIsLoaded()) {
      ...
      document.getElementById('edit-script-btn')!.setAttribute('disabled', 'true')
    }
    ...
    if (!result) {
      this.gameContent.innerHTML = `
        <div class="welcome-screen">
          ...
          ${this.loadedDLCs.size > 0 ? `<p style="margin-top: 12px;"><strong>🎉 DLC Active:</strong> ${this.loadedDLCs.size} expansion${this.loadedDLCs.size > 1 ? 's' : ''} loaded!</p>` : ''}
        </div>
      `
      return
    }
  ...
  private renderChoices(result: any): void {
    ...
    if (result.choices) {
      ...
      result.choices.forEach((choice: any, index: number) => {
        ...
        choiceButton.addEventListener('click', () => this.vnEngine.makeChoice(index))
  ...
  private renderGameState(): void {
    if (!this.vnEngine.getIsLoaded()) {
      this.gameState.innerHTML = '<div style="opacity: 0.7;">No script loaded</div>'
      return
    }
    ...
    this.gameState.innerHTML = `
      <div style="color: #2c3e50; line-height: 1.4; font-size: 11px;">
        <div style="margin-bottom: 8px;">
          ...
          <span style="color: #667eea;">${gameState.currentScene || 'none'}</span>
        </div>
        ...
        <div style="margin-bottom: 8px;">
          <strong>Story Flags (${(gameState.storyFlags || []).length}):</strong><br>
  ...
  private autoResizeTextarea(): void {
    ...
    this.yamlEditor.style.height = Math.max(200, this.yamlEditor.scrollHeight) + 'px'
  }
