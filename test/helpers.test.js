...
export async function runHelperTests() {
  ...
  // VN Helper Tests
  test('VN Helpers - Flag Management', async () => {
    reporter.startTest('VN Helpers - Flag Management');
    ...
    // Flag operations
    assert.assert(!helpers.vn.hasFlag('testFlag', mockContext), 'hasFlag() should initially return false', false, helpers.vn.hasFlag('testFlag', mockContext), {
      context: mockContext, flag: 'testFlag'
    });
    
    helpers.vn.addFlag('testFlag', mockContext);
    assert.assert(helpers.vn.hasFlag('testFlag', mockContext), 'hasFlag() should return true after adding', true, helpers.vn.hasFlag('testFlag', mockContext), {
      context: mockContext, flag: 'testFlag'
    });
    
    helpers.vn.removeFlag('testFlag', mockContext);
    assert.assert(!helpers.vn.hasFlag('testFlag', mockContext), 'hasFlag() should return false after removing', false, helpers.vn.hasFlag('testFlag', mockContext), {
      context: mockContext, flag: 'testFlag'
    });
    
    const toggleResult = helpers.vn.toggleFlag('testFlag', mockContext);
    assert.assert(toggleResult, 'toggleFlag() should return true when adding', true, toggleResult, {
      context: mockContext, flag: 'testFlag'
    });
    
    const toggleResult2 = helpers.vn.toggleFlag('testFlag', mockContext);
    assert.assert(!toggleResult2, 'toggleFlag() should return false when removing', false, toggleResult2, {
      context: mockContext, flag: 'testFlag'
    });
    
    console.log('   ✅ VN flag management working');
  });

  test('VN Helpers - Variable Management', async () => {
    reporter.startTest('VN Helpers - Variable Management');
    reporter.setContext({ subcategory: 'vn-helpers' });
    ...
    // Variable operations
    assert.assertEqual(helpers.vn.getVar('testVar', 'default', mockContext), 'default', 'getVar() should return default for missing var', {
      context: mockContext, key: 'testVar', defaultValue: 'default'
    });
    
    helpers.vn.setVar('testVar', 42, mockContext);
    assert.assertEqual(helpers.vn.getVar('testVar', 'default', mockContext), 42, 'getVar() should return set value', {
      context: mockContext, key: 'testVar'
    });
    
    assert.assert(helpers.vn.hasVar('testVar', mockContext), 'hasVar() should detect existing variable', true, helpers.vn.hasVar('testVar', mockContext), {
      context: mockContext, key: 'testVar'
    });
    
    assert.assert(!helpers.vn.hasVar('nonExistentVar', mockContext), 'hasVar() should not detect missing variable', false, helpers.vn.hasVar('nonExistentVar', mockContext), {
      context: mockContext, key: 'nonExistentVar'
    });
    
    const incrementResult = helpers.vn.incrementVar('testVar', 8, mockContext);
    assert.assertEqual(incrementResult, 50, 'incrementVar() should add to existing value', {
      context: mockContext, key: 'testVar', amount: 8, result: incrementResult
    });
    
    // Test nested variables
    helpers.vn.setVar('player.name', 'Alice', mockContext);
    assert.assertEqual(helpers.vn.getVar('player.name', null, mockContext), 'Alice', 'setVar() should handle nested properties', {
      context: mockContext, key: 'player.name'
    });
    
    console.log('   ✅ VN variable management working');
  });

  test('VN Helpers - Choice History', async () => {
    reporter.startTest('VN Helpers - Choice History');
    reporter.setContext({ subcategory: 'vn-helpers' });
    ...
    const mockContext = {
      ...
      variables: {},
      choiceHistory: [
        { scene: 'scene1', choiceText: 'Go left', timestamp: Date.now() },
        { scene: 'scene2', choiceText: 'Talk to NPC', timestamp: Date.now() }
      ],
      computed: {
        gameTime: '00:00',
        hasFlag: () => false,
    ...
    // Choice history operations
    assert.assert(helpers.vn.playerChose('Go left', null, mockContext), 'playerChose() should find choice in history', true, helpers.vn.playerChose('Go left', null, mockContext), {
      context: mockContext, choiceText: 'Go left'
    });
    
    assert.assert(!helpers.vn.playerChose('Go right', null, mockContext), 'playerChose() should not find non-existent choice', false, helpers.vn.playerChose('Go right', null, mockContext), {
      context: mockContext, choiceText: 'Go right'
    });
    
    assert.assert(helpers.vn.playerChose('Go left', 'scene1', mockContext), 'playerChose() should find choice in specific scene', true, helpers.vn.playerChose('Go left', 'scene1', mockContext), {
      context: mockContext, choiceText: 'Go left', scene: 'scene1'
    });
    
    assert.assert(!helpers.vn.playerChose('Go left', 'scene2', mockContext), 'playerChose() should not find choice in wrong scene', false, helpers.vn.playerChose('Go left', 'scene2', mockContext), {
      context: mockContext, choiceText: 'Go left', scene: 'scene2'
    });
    
    const lastChoice = helpers.vn.getLastChoice(mockContext);
    assert.assertEqual(lastChoice.choiceText, 'Talk to NPC', 'getLastChoice() should return most recent choice', {
      context: mockContext, result: lastChoice
    });
    
    assert.assertEqual(helpers.vn.choiceCount(mockContext), 2, 'choiceCount() should return correct count', {
      context: mockContext, result: helpers.vn.choiceCount(mockContext)
    });
    
    console.log('   ✅ VN choice history working');
  });
